...
def get_invite():
    ...
    if not re.match(r'^1[3456789]\d{9}$', phone):
        return jsonify(jsonify(success=RET.WRONG, data='用户手机号格式错误'))
    # 保存用户实名信息到sqlserver数据库中
    # 构造模型类对象,准备保存用户信息
    invitation = Invitation(full_name=full_name, phone=phone, visitor_count=visitor_count, visit_time=visit_time,
                            position=position, reason=reason, check_in=check_in, user_id=user_id,
                            state="待接受")
    # 提交数据到数据库中
    try:
    ...
    # 返回结果
    return jsonify(success=RET.OK, data={"invite_id": invitation.id,  # 该请柬id
...
#         # invit = Invitation.query.filter_by(user_id=user_id).order_by(Invitation.update_time.desc()).limit(10)    #  通过id查询的邀请函表
#         # list1 =[]
#         # for i in invit:
#         #     list1.append(i.inviting_object())
#         # applicant = Applicant.query.filter_by(user_id=user_id).order_by(Invitation.update_time.desc()).limit(10)  #   通过id查询的申请人的表
#         # for k in applicant:
#         #     list1.append(k.invit_object())
#         # return jsonify(success=RET.OK, data=list1)
#         from sqlalchemy.sql import union_all
#         invit = union_all(Invitation.select().
#         where(Invitation.user_id == user_id).order_by(Applicant.update_time.desc()),Applicant.select().
#         where(Applicant.user_id == user_id).order_by(Applicant.update_time.desc()),).limit(20)
#         a = conn.execute(invit).fetchall()
#
#     except Exception as e:
#         current_app.logger.error(e)
